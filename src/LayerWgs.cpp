...
LayerWgs::LayerWgs(Network *net, int inputs, int outputs, 
    ...
    int seek = 0;
    readBinaryFileOrRandomInit(weights_path.c_str(), inputs*outputs*kh*kw*kl, &data_h, &data_d, seek);
    seek += inputs*outputs*kh*kw*kl;
    this->additional_bias = additional_bias;
    if(additional_bias) {
        readBinaryFileOrRandomInit(weights_path.c_str(), outputs, &bias2_h, &bias2_d, seek);
        seek += outputs;   
    }
    
    readBinaryFileOrRandomInit(weights_path.c_str(), outputs, &bias_h, &bias_d, seek);

    this->batchnorm = batchnorm;
    if(batchnorm) {
        seek += outputs;
        readBinaryFileOrRandomInit(weights_path.c_str(), outputs, &scales_h, &scales_d, seek);
        seek += outputs;
        readBinaryFileOrRandomInit(weights_path.c_str(), outputs, &mean_h, &mean_d, seek);
        seek += outputs;
        readBinaryFileOrRandomInit(weights_path.c_str(), outputs, &variance_h, &variance_d, seek);
