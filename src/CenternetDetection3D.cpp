...
bool CenternetDetection3D::init(const std::string& tensor_path, const int n_classes, const int n_batches, 
    ...
    checkCuda( cudaMallocHost((void **) &xs, K  * sizeof(float)) );
    checkCuda( cudaMallocHost((void **) &ys, K  * sizeof(float)) );
    ...
    checkCuda( cudaMalloc((void **) &srcOut, K *sizeof(float)) );
    checkCuda( cudaMalloc((void **) &idsOut, K *sizeof(int)) );
