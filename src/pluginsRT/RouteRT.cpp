...
RouteRT::RouteRT(const void *data, size_t length) {
    ...
    h = readBUF<int>(buf);
    w = readBUF<int>(buf);
	mDataType = readBUF<DataType>(buf);
...
int RouteRT::enqueue(int batchSize, const void *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) NOEXCEPT {
...
void RouteRT::serialize(void *buffer) const NOEXCEPT {
    ...
    writeBUF(buf, h);
    writeBUF(buf, w);
	writeBUF(buf, mDataType);
    assert(buf == a + getSerializationSize());
}

bool RouteRT::supportsFormatCombination(int pos, const PluginTensorDesc* inOut, int nbInputs, int nbOutputs) const NOEXCEPT {
...
DataType RouteRT::getOutputDataType(int index, const nvinfer1::DataType* inputTypes, int nbInputs) const NOEXCEPT {
...
IPluginV2IOExt *RouteRT::clone() const NOEXCEPT {
    auto *p = new RouteRT(*this);
    p->setPluginNamespace(mPluginNamespace.c_str());
    return p;
}

bool RouteRT::isOutputBroadcastAcrossBatch(int outputIndex, const bool* inputIsBroadcasted, int nbInputs) const NOEXCEPT {
    return false;
}

bool RouteRT::canBroadcastInputAcrossBatch(int inputIndex) const NOEXCEPT {
    return false;
}

void RouteRT::configurePlugin(const PluginTensorDesc* inTensor, int nbInputs, const PluginTensorDesc* outTensor, int nbOutput) NOEXCEPT {
