...
__global__ void shortcut_mul_kernel_half(int size, int minw, int minh, int minc, int sample, int batch, 
                                int w1, int h1, int c1, __half *mul, 
                                int w2, int h2, int c2, __half s1, __half s2, __half *out)
{
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= size) return;
    ...
    int out_index = i*sample + w1*(j*sample + h1*(k + c1*b));
    // out[out_index] = out[out_index] * mul[k + c2*b];
    out[out_index] = __hmul(out[out_index], mul[k + c2*b]);
}
                                                        ...
                                                        bool mul, cudaStream_t stream)
{
	...
	if(!mul){
        ...
        shortcut_mul_kernel_half<<<blocks, threads, 0, stream>>>(size, minw, minh, minc, sample, batch, 
            w1, h1, c1, srcData, w2, h2, c2, __float2half((float)s1), __float2half((float)s2), dstData);  
	}
}
